...
describe('PgCalendarioModuloRepository', () => {
  ...
  it('createMany sends one INSERT per entry, ON CONFLICT DO NOTHING', async () => {
    ...
    await repo.createMany([
      { academicYearModuleId: 'am1', category: 'holidays', name: 'Vacaciones de Navidad.', startDate: '2026-12-22', endDate: '2027-01-07' },
      { academicYearModuleId: 'am1', category: 'evaluations', name: 'Evaluación.', startDate: '2027-03-01', endDate: '2027-03-01' },
    ]);
    ...
    expect(fakeSql.calls[0]!.values).toEqual(['am1', 'holidays', 'Vacaciones de Navidad.', '2026-12-22', '2027-01-07']);
    expect(fakeSql.calls[1]!.values).toEqual(['am1', 'evaluations', 'Evaluación.', '2027-03-01', '2027-03-01']);
  });
