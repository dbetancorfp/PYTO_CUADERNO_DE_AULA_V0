...
// GET /api/calendario-horario (findForTeacher). Non-working ranges are derived from this
// same módulo's own already-seeded calendario_modulo rows (holidays/public_holidays/
// free_disposal_days only — academic_key_dates is informational, same exclusion
// calendario-modulo.service.ts's nonWorkingRangesFor already applies for UC-08/UC-09), not
// from key_dates directly.
...
describe('elementId: calendario-months, calendario-legend, calendario-day-tooltip (seedForModule, UC-12)', () => {
  it('inserts one row per laborable school-year date matching a scheduled weekday', async () => {
    const deps = fakeDeps({ moduloEntries: [] });
    const service = makeService(deps);
    const schedule: AcademicYearModuleScheduleEntry[] = [{ weekday: 1, hours: 2 }]; // Monday

    await service.seedForModule('am1', 2026, schedule);

    expect(deps.replaceAllCalls).toHaveLength(1);
    expect(deps.replaceAllCalls[0]!.academicYearModuleId).toBe('am1');
    // 2026-09-07 is the first Monday of the 2026-2027 school year (2026-09-01 is a Tuesday).
    expect(deps.replaceAllCalls[0]!.entries).toContainEqual({ date: '2026-09-07', hours: 2 });
  });

  it('excludes a scheduled weekday date that falls inside a holidays/public_holidays/free_disposal_days range', async () => {
    const deps = fakeDeps({
      moduloEntries: [makeModuloEntry({ category: 'holidays', startDate: '2026-12-22', endDate: '2027-01-07' })],
    });
    const service = makeService(deps);
    const schedule: AcademicYearModuleScheduleEntry[] = [{ weekday: 1, hours: 2 }]; // Monday
    ...
    // The next non-holiday Monday (2027-01-11) is still included.
  ...
  it('every inserted date falls within the school year window (1 September startYear - 30 June startYear+1)', async () => {
    const deps = fakeDeps({ moduloEntries: [] });
    const service = makeService(deps);
    ...
    expect(dates.every((date) => date >= '2026-09-01' && date <= '2027-06-30')).toBe(true);
    expect(dates.some((date) => date > '2027-06-30')).toBe(false);
  });
  ...
  it('an empty schedule replaces with an empty array (clears any previously generated rows)', async () => {
    ...
    await service.seedForModule('am1', 2026, []);
  ...
  it('each entry gets the hours value of its own matching weekday, not a single shared value', async () => {
    ...
    expect(entries).toContainEqual({ date: '2026-09-07', hours: 1 }); // Monday
    expect(entries).toContainEqual({ date: '2026-09-11', hours: 3 }); // Friday
  });
});
...
describe('elementId: calendario-months, calendario-legend, calendario-day-tooltip (findForTeacher, UC-13)', () => {
  it('returns the calendario_horario entries for a módulo owned by this teacher', async () => {
    const entries: CalendarioHorarioEntry[] = [{ date: '2026-09-07', hours: 2 }];
